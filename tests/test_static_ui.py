...
class StaticUiTests(unittest.TestCase):
    ...
    def test_static_editor_shell_routes_are_public_and_do_not_mutate(self) -> None:
        ...
        self.assertIn("const currentRequest = isCurrentSaveRequest(requestId, documentId, baseVersion, contentText)", save_loader)
        self.assertIn("if (!currentRequest)", save_loader)
        self.assertIn("queueOfflineSave(savePayload)", save_loader)
        self.assertLess(save_loader.index("if (!currentRequest)"), save_loader.index("queueOfflineSave(savePayload)"))
