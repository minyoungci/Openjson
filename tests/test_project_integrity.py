...
from app.integrity_service import check_project_replay_integrity
from app.main import create_app
from app.workspace_service import add_project_member, create_project, create_user, create_workspace


class ProjectReplayIntegrityTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_project_integrity_reports_ok_for_patch_rollback_delete_and_restore_without_mutation(self) -> None:
        ...
        result = check_project_replay_integrity(
            self.db_path,
            project_id=self.project["id"],
            actor_id=self.owner["id"],
        )
        ...
        self.assertEqual(result["documents"][0]["document_id"], document["id"])
        self.assertEqual(result["documents"][0]["current_version"], 5)
        self.assertEqual(result["documents"][0]["latest_event_version"], 5)
        self.assertEqual(result["documents"][0]["event_count"], 5)
        self.assertEqual(result["documents"][0]["status"], "ok")
        self.assertEqual(result["documents"][0]["replay_matches_latest"], True)
        self.assertEqual(self._event_count(), before_events)
        self.assertEqual(self._audit_count(), before_audit)

    def test_project_integrity_detects_snapshot_and_version_tampering(self) -> None:
        ...
        failures_by_id = {failure["document_id"]: failure for failure in result["failures"]}
        self.assertEqual(failures_by_id[snapshot_tampered["id"]]["error_code"], "SNAPSHOT_REPLAY_MISMATCH")
        self.assertEqual(failures_by_id[version_tampered["id"]]["error_code"], "VERSION_MISMATCH")
        self.assertEqual(failures_by_id[version_tampered["id"]]["latest_event_version"], 3)
        self.assertEqual(failures_by_id[version_tampered["id"]]["current_version"], 99)
    ...
    def test_include_deleted_policy(self) -> None:
        ...
        default_result = check_project_replay_integrity(
            self.db_path,
            project_id=self.project["id"],
            actor_id=self.owner["id"],
        )
        active_only = check_project_replay_integrity(
            self.db_path,
            project_id=self.project["id"],
            actor_id=self.owner["id"],
    ...
    def test_project_integrity_permission_policy_owner_admin_only(self) -> None:
    ...
    def test_http_route_and_api_token_scope(self) -> None:
        ...
        checked = client.get(
            f"/projects/{self.project['id']}/integrity/replay",
            headers={"Authorization": f"Bearer {owner_token}"},
        )
        other_project_check = client.get(
            f"/projects/{self.other_project['id']}/integrity/replay",
            headers={"Authorization": f"Bearer {owner_token}"},
        )
    ...
    def test_project_integrity_route_is_registered(self) -> None:
        app = create_app(self.db_path)
        routes = {(route.path, ",".join(sorted(route.methods))) for route in app.routes if hasattr(route, "methods")}

        self.assertIn(("/projects/{project_id}/integrity/replay", "GET"), routes)
